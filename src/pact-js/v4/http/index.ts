...
import { ConsumerInteraction, ConsumerPact } from '../../pact.d.ts';
import { JsonMap } from '../../common/jsonTypes.ts';
import { forEachObjIndexed } from 'npm:ramda';
import { Path, TemplateHeaders, TemplateQuery, V3MockServer } from '../../v3/types.ts';
import { AnyTemplate, Matcher, matcherValueOrString } from '../../v3/matchers.ts';
import {
  PactV4Options,
  PluginConfig,
...
const readBinaryData = (file: string): Buffer => {
  try {
    const body = Deno.readFileSync(file);
...
const cleanup = (
  ...
  if (success) {
    pact.writePactFileForPluginServer(server.port,opts.dir || './pacts');
  }
